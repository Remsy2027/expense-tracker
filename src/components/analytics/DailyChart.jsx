...
const DailyChart = ({
  ...
  const chartData = [
    {
      ...
      change: previousData
        ? calculations.totalIncome - (previousData.totalIncome || 0)
        : 0,
    },
    {
      name: "Expenses",
      ...
      change: previousData
        ? calculations.totalExpenses - (previousData.totalExpenses || 0)
        : 0,
    },
    {
      name: "Balance",
      ...
      change: previousData
        ? calculations.balance - (previousData.balance || 0)
        : 0,
    },
  ];
        ...
        <div className="bg-white border border-gray-200 rounded-lg shadow-lg p-4 max-w-xs">
          ...
          {showComparison && data.change !== 0 && (
            ...
            >
              ...
              <span>
                {data.change > 0 ? "+" : ""}
                {formatCurrency(Math.abs(data.change))} vs yesterday
              </span>
            </div>
          )}
        </div>
  ...
  const getPercentage = (value) =>
    totalAmount > 0 ? ((Math.abs(value) / totalAmount) * 100).toFixed(1) : 0;
          ...
          <div className="flex items-start space-x-2">
            ...
            <div className="flex-1">
              ...
              <div className="space-y-1 text-sm text-gray-600">
                {calculations.balance > 0 ? (
                  <p>
                    ✅ You're in the green today with a positive balance of{" "}
                    {formatCurrency(calculations.balance)}
                  </p>
                ) : calculations.balance < 0 ? (
                  <p>
                    ⚠️ You've overspent by{" "}
                    {formatCurrency(Math.abs(calculations.balance))} today
                  </p>
                ) : (
                  <p>⚖️ You've broken even today with no surplus or deficit</p>
                )}

                {calculations.totalExpenses > calculations.totalIncome &&

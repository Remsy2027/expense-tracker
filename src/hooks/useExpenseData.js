...
export function useExpenseData(currentDate) {
  ...
  const addExpense = useCallback(async (expenseData) => {
    ...
    try {
      const result = contextAddExpense(currentDate, expenseData || expenseForm);
  ...
  }, [currentDate, expenseForm, contextAddExpense, addNotification, isSubmitting]);

  const deleteTransaction = useCallback(async (transactionId, transactionType) => {
    try {
      const result = contextDeleteTransaction(currentDate, transactionId, transactionType);
  ...
  const updateTransaction = useCallback(async (transactionId, transactionType, updatedData) => {
    try {
      const result = contextUpdateTransaction(currentDate, transactionId, transactionType, updatedData);
  ...
  const addQuickIncome = useCallback((amount, source = 'Quick Income') => {
    return addIncome({ amount, source });
  }, [addIncome]);

  const addQuickExpense = useCallback((amount, description = 'Quick Expense', category = 'Other') => {
  ...
  const getSpendingTrend = useCallback((days = 30) => {
  ...
  const getCategoryPercentages = useCallback(() => {
    ...
    const percentages = {};
    Object.entries(calculations.categoryTotals || {}).forEach(([category, amount]) => {
  ...
  const searchTransactions = useCallback((query, type = 'all') => {
    ...
    if (type === 'all' || type === 'income') {
      const incomeResults = currentData.income.filter(item =>
    ...
    if (type === 'all' || type === 'expenses') {
      const expenseResults = currentData.expenses.filter(item =>
  ...
  const filterTransactionsByCategory = useCallback((category) => {
    const currentData = dailyData[currentDate] || { income: [], expenses: [] };
    return currentData.expenses.filter(expense => expense.category === category);
  }, [dailyData, currentDate]);

  const filterTransactionsByAmount = useCallback((minAmount, maxAmount) => {
    const currentData = dailyData[currentDate] || { income: [], expenses: [] };

...
import { PieChart as PieChartIcon, BarChart3, TrendingDown, Eye, EyeOff } from 'lucide-react';
...
const CategoryChart = ({ 
  ...
  const visibleData = categoryData.filter(item => !hiddenCategories.has(item.name));
      ...
      <div className="px-6 pb-6">
        <div className="bg-amber-50 border border-amber-200 rounded-lg p-4">
          <div className="flex items-start space-x-2">
            ...
            <div>
              <h4 className="text-sm font-medium text-amber-800 mb-1">Spending Insights</h4>
              <div className="text-sm text-amber-700 space-y-1">
                {categoryData.length > 0 && (
                  <p>
                    Your highest expense category is <strong>{categoryData[0].name}</strong> 
                    accounting for <strong>{categoryData[0].percentage}%</strong> of total spending.
                  </p>
                )}
                {categoryData.length > 2 && (
                  <p>
                    Top 3 categories represent <strong>
                    {categoryData.slice(0, 3).reduce((sum, cat) => sum + parseFloat(cat.percentage), 0).toFixed(1)}%
                    </strong> of your expenses.
                  </p>
                )}
              </div>
            </div>

...
export const validateTransaction = (transaction, type = "expense") => {
  ...
  if (!transaction.amount || isNaN(parseFloat(transaction.amount))) {
    ...
    if (amount < VALIDATION_RULES.TRANSACTION_AMOUNT.min) {
      errors.push(
        `Amount must be at least ${VALIDATION_RULES.TRANSACTION_AMOUNT.min}`,
      );
    }
    if (amount > VALIDATION_RULES.TRANSACTION_AMOUNT.max) {
      errors.push(
        `Amount cannot exceed ${VALIDATION_RULES.TRANSACTION_AMOUNT.max}`,
      );
    }
  }
  ...
  if (!text || text.trim().length === 0) {
    errors.push(
      `${textField.charAt(0).toUpperCase() + textField.slice(1)} is required`,
    );
  } else {
    if (text.length < VALIDATION_RULES.DESCRIPTION.minLength) {
      errors.push(
        `${textField.charAt(0).toUpperCase() + textField.slice(1)} must be at least ${VALIDATION_RULES.DESCRIPTION.minLength} character`,
      );
    }
    if (text.length > VALIDATION_RULES.DESCRIPTION.maxLength) {
      errors.push(
        `${textField.charAt(0).toUpperCase() + textField.slice(1)} cannot exceed ${VALIDATION_RULES.DESCRIPTION.maxLength} characters`,
      );
    }
  }
...
export const validateDate = (dateString) => {
  ...
  if (date < minDate) {
    return {
      isValid: false,
      error: `Date cannot be before ${formatDate(minDate)}`,
    };
  }
  ...
  if (date > maxDate) {
    return {
      isValid: false,
      error: `Date cannot be after ${formatDate(maxDate)}`,
    };
  }
...
export const sanitizeInput = (input, options = {}) => {
  const { allowSpecialChars = false, maxLength = 1000, trim = true } = options;
...
export const calculateMonthlyStats = (dailyData, date) => {
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const dayIncome = dayData.income.reduce(
      (sum, item) => sum + item.amount,
      0,
    );
    const dayExpenses = dayData.expenses.reduce(
      (sum, item) => sum + item.amount,
      0,
    );

...
import { TrendingUp, TrendingDown, DollarSign, List, Calendar, Target, AlertCircle, CheckCircle } from 'lucide-react';
import { formatCurrency, calculatePercentageChange, formatPercentage } from '../../utils/helpers';
...
const SummaryCards = ({ 
  ...
  const cards = [
    {
      ...
      trend: getTrendData(calculations.totalIncome, previousDayData?.totalIncome, 'income')
    },
    {
      id: 'expenses',
      ...
      trend: getTrendData(calculations.totalExpenses, previousDayData?.totalExpenses, 'expenses')
    },
    {
      id: 'balance',
      title: 'Net Balance',
      ...
      icon: DollarSign,
      gradient: calculations.balance >= 0 ? 'from-blue-500 to-blue-600' : 'from-orange-500 to-orange-600',
      bgColor: calculations.balance >= 0 ? 'bg-blue-50' : 'bg-orange-50',
      textColor: calculations.balance >= 0 ? 'text-blue-700' : 'text-orange-700',
      iconColor: calculations.balance >= 0 ? 'text-blue-600' : 'text-orange-600',
      trend: getTrendData(calculations.balance, previousDayData?.balance, 'balance')
    },
    {
      id: 'transactions',
      title: 'Transactions',
      ...
      isCount: true,
      trend: getTrendData(calculations.transactionCount, previousDayData?.transactionCount, 'transactions')
    }
  ];

  // Additional monthly cards if showing goals
  const monthlyCards = showGoals && monthlyGoals ? [
    {
      id: 'monthly-income',
  ...
  const getProgressStatus = (current, target, type = 'income') => {
    ...
    if (type === 'expenses') {
      if (percentage <= 80) return { status: 'success', icon: CheckCircle, message: 'Under budget' };
      if (percentage <= 100) return { status: 'warning', icon: AlertCircle, message: 'Near budget limit' };
      return { status: 'danger', icon: AlertCircle, message: 'Over budget' };
    } else {
      if (percentage >= 100) return { status: 'success', icon: CheckCircle, message: 'Goal achieved' };
      if (percentage >= 80) return { status: 'warning', icon: AlertCircle, message: 'Close to goal' };
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {allCards.map((card) => {
        ...
        const progress = hasTarget ? getProgressPercentage(card.value, card.target) : null;
        const progressStatus = hasTarget ? getProgressStatus(card.value, card.target, card.id.includes('expense') ? 'expenses' : 'income') : null;
        ...
        return (
          ...
          >
                  ...
                  <div className="mt-2">
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2">
                      <div
                        className={`h-2 rounded-full transition-all duration-300 ${
                          progressStatus?.status === 'success' ? 'bg-green-500' :
                          progressStatus?.status === 'warning' ? 'bg-yellow-500' :
                          progressStatus?.status === 'danger' ? 'bg-red-500' :
                          'bg-blue-500'
                        }`}
                    ...
                    {progressStatus && (
                      <div className={`flex items-center space-x-1 mt-2 text-xs ${
                        progressStatus.status === 'success' ? 'text-green-600' :
                        progressStatus.status === 'warning' ? 'text-yellow-600' :
                        progressStatus.status === 'danger' ? 'text-red-600' :
                        'text-blue-600'
                      }`}>
            ...
            <div className={`absolute top-0 right-0 w-16 h-16 transform translate-x-8 -translate-y-8 bg-gradient-to-br ${card.gradient} opacity-10 rounded-full`} />
          </div>
        );
      })}
    </div>
  ...
  return (
    <div className="grid grid-cols-3 gap-4">
      {cards.map((card, index) => (
        <div key={index} className={`${card.bgColor} rounded-lg p-4 text-center`}>
